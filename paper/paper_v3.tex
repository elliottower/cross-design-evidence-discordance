...
\title{Cross-Design Evidence Discordance Diagnoses Phase~III Drug Failure Modes Across Neurodegenerative and Cardiometabolic Disease}
...
\begin{abstract}
The overall failure rate for AD drug development exceeds 99\% \citep{cummings2014}, yet evidence synthesis tools assess consistency within a single study design rather than across designs. We define a cross-design concordance rule: for each mechanism family, we standardize effect sizes from observational and Mendelian randomization (MR) evidence onto Cohen's $d$ and classify a family as \emph{discordant} when a non-trivial observational association ($d \geq 0.10$) coincides with a null MR signal ($d < 0.10$ or confidence interval spanning the null). No interventional data enter the classification.

An ablation analysis shows that MR-null status alone accounts for all predictive power. Across 22 scored families in three domains---8 neuroepidemiology (AD, MS), 7 cardiometabolic, 7 autoimmune---MR-only achieves 18/22 correct classifications (vs.\ 11/22 for an always-predict-failure baseline); the cross-design rule matches MR-only exactly (zero McNemar disagreements). Observational evidence is non-trivial ($d \geq 0.10$) for every scored family and adds no discriminative information. This has a structural explanation: mechanisms that reach Phase~III have strong observational evidence by construction---the pharmaceutical industry does not invest in targets lacking epidemiological support. The 18/22 total combines domains with different observational constructs (epidemiological ORs for neuro/cardio, case-control SMDs for autoimmune); domain-specific accuracies are 7/8, 7/7, and 4/7 respectively and are not pooled for inference.

The cross-design framework contributes something MR alone cannot: \emph{failure-mode diagnosis}. A two-stage analysis---first etiologic evidence only, then adding RCT outcomes---reveals three failure modes. \emph{Zombie mechanisms} (null MR, confounded observational signal) indicate the target was never causal; drugs fail because there is nothing to intervene on (e.g., metabolic risk factors for AD, HRT for AD, HDL-raising for CVD). \emph{Translation-gap mechanisms} (concordant etiologic evidence, but RCT null) indicate the target is genuinely causal but the intervention cannot reverse established pathology (e.g., amyloid immunotherapy in AD). \emph{Exposure mismatch} (concordant etiologic evidence, but the MR instrument captures a different exposure window than the drug) accounts for the single concordance miss (vitamin~D supplementation for MS). MR-only predicts all three will fail; only the cross-design framework distinguishes \emph{why}, and each demands a different response: zombies should be retired from pipelines, translation gaps call for different therapeutic strategies, and exposure mismatches signal that the target is real but the intervention does not recapitulate the causal exposure.

The autoimmune domain reveals a mechanistically predictable boundary: three approved effector-neutralization drugs (anti-TNF, anti-IL-17, abatacept) show null MR because their targets are elevated as a \emph{consequence} of disease, not a cause. This defines where MR-based screening is expected to misclassify---soluble disease effectors vs.\ germline risk-encoding loci. Two cardiometabolic families with concordant etiologic evidence and pending Phase~III readouts (Lp(a)-lowering, IL-6 pathway inhibition) constitute pre-registered prospective predictions.
\end{abstract}

\section{Introduction}

Drug development for Alzheimer's disease and multiple sclerosis consumes billions of dollars annually, with an overall AD drug failure rate exceeding 99\% \citep{cummings2014}. Standard tools for evaluating mechanistic evidence---GRADE for rating certainty, $I^2$ for measuring heterogeneity---operate within a single study design. They assess whether twelve RCTs agree, leaving cross-design consistency untested: whether RCT evidence agrees with genetic evidence, or whether observational signals survive Mendelian randomization.

Several independent findings establish that genetic evidence carries information about drug success. \citet{nelson2015} found that drugs with genetic support for their target are twice as likely to reach approval, a finding refined by \citet{minikel2024} with larger datasets. \citet{gill2021} showed that MR concordance with RCTs predicts drug approval. \citet{ference2019} demonstrated that MR-derived causal estimates for cardiovascular targets match RCT efficacy. Triangulation \citep{lawlor2016} captures the intuition that multiple evidence types with different biases should converge on a true effect, but provides no standard quantitative test and does not distinguish \emph{types} of convergence failure.
...
\textbf{Anti-CD20-MS.} MR evidence shows that per-SD higher circulating FCRL3 protein is protective against MS (OR~0.83, 95\% CI 0.79--0.89; \citealp{lin2023}). Observational B-cell depletion efficacy data from \citet{hu2019}. A CD40 genetic association (OR~1.16; \citealp{sokolova2013}) provides corroborating GWAS evidence.
...
Two mechanistically defined failure modes follow from the two-stage analysis:
...
$^\dagger$BMI~$\to$~MS: MR evidence addresses MS susceptibility \citep{mokry2016obesity}; no trial tests a matching clinical endpoint. Excluded from scored count.}
\end{table}
...
Triangulation \citep{lawlor2016} captures the right intuition---multiple evidence types with different biases should converge on a true effect---but provides no quantitative decision rule. Our contribution is complementary: triangulation is about \emph{convergence} boosting credibility; the concordance rule is about \emph{divergence} diagnosing failure mode.
