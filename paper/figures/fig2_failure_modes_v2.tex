...
% ===== PANEL a — MR-discordant mechanism =====
\node[bold] (aObs) {Observational\\association};
\node[bold, right=10mm of aObs] (aMR) {MR / genetic\\signal};
\node[zombie, below=14mm of aObs] (aConf) {Confounding or\\reverse causation\\(attributed)};
\node[neut, below=14mm of aMR] (aNull) {Null MR\\estimate};

\coordinate (aMid) at ($(aConf)!0.5!(aNull)$);
\node[zombie, below=14mm of aMid] (aDrug) {Drug targets pathway\\without MR support};
...
% Panel label
\node[panel, above=4mm of $(aObs.north)!0.5!(aMR.north)$] {\textbf{a}\quad MR-discordant mechanism};
