...
\matrix[matrix of nodes, anchor=north west, at={($(cobs.south)+(8mm,-18mm)$)},
  ...
  \node[lab, text=genCol, font=\sffamily\footnotesize\bfseries] (ccaus) {MR supportive}; \\
  \node[lab, text=obsCol, font=\sffamily\footnotesize\bfseries, align=right] (rnt) {OBS non-trivial}; &
  \node[cell, fill=qualCol!15, draw=qualCol, text=qualCol] (q) {Qualitative discordance\\$\to$ expected failure}; &
  \node[cell, fill=concCol!20, draw=concCol, text=concCol!70!black] (c) {Concordance\\$\to$ expected success}; \\
  ...
  \node[cell, fill=genoCol!12, draw=genoCol, text=genoCol] (g) {Genetic-only\\$\to$ expected success}; \\
};
...
\node[box, fill=bgGray, draw=black!40, minimum width=44mm, minimum height=12mm] (read)
  at ($(cellmid)+(0,-24mm)$) {classification vs.\ outcome;\\failure-mode reading};
