# MechVal — Alzheimer's Disease / Neurodegeneration Mechanism Catalog (Xia Program, Domain 2)

**Build date:** 2026-06-30  |  **Status:** INITIALIZING SCAFFOLD (to be filled iteratively, like the MS catalog)
**Companion to:** MechVal_MS_Neurology_Catalog_Xia.md (Domain 1, 52 cases)
...
MechVal families/operators recur in AD is the strongest possible generalization argument.
...
RE-DERIVES from an independent literature. If it does, MechVal is a method, not an MS artifact. Early
...
multi-axis (not linear-cascade) progression structure that predicts H^1 != 0 — then MechVal is a
...
**Tier assignment:** c1 **T4** (universally observed). c2 **T3** (amyloid-first temporal ordering in
biomarker cascade) [web:937]. c3 **T2-T3, CONTESTED** — causal INITIATION supported but SUFFICIENCY
disputed (poor plaque-cognition link) [web:929][web:934]. c4 **T2** (modest benefit; see C1).

**Verdict:** **SUPPORTED that amyloid is AN early causal contributor; DISCONFIRMED as a SUFFICIENT linear
cause; the strong-form cascade is CONTESTED — necessary-ish but not sufficient.**
...
**Tier assignment:** c1 **T4**. c2 **T4** (dose-response, huge samples) [web:952]. c3 **T3-T4**
(multi-mechanism, incl. causal tau effect) [web:935]. c4 **T3** (modifies onset age & progression).
...
**Tier assignment:** c1 **T3-T4** (R47H is a well-replicated risk variant). c2 **T3** (proteomic MR)
[web:930]. c3 **T3** (MR + postmortem validation) [web:930]. c4 **T2-T3** (causal-driver vs
...
**Tier assignment:** c1 **T2-T3** (consistent association). c2 **DISCONFIRMED by MR** — well-powered null
...
**Tier assignment:** c1 **T3** (Lancet Commission-style epidemiology). c2 **T2-T3, MIXED by factor**
(some MR-causal, some confounded) [web:948]. c3 **T2-T3**. c4 **T2** (population-attributable estimates
are model-dependent).
...
This is the headline forming: MechVal's family structure and MR-discrimination method REPRODUCE in AD.
...
**Tier assignment:** c1 **T4**. c2 **T3-T4** (Centiloid harmonization strong, r^2>0.9) [web:958]. c3
**T3** (early substrate). c4 **T2-T3, SOFT** — the threshold (20-25 CL band) is a BRACKET, not a sharp
...
**Tier assignment:** c1 **T3**. c2 **T2** — off-target binding (iron, MAO-B, skull) substantially
contaminates signal [web:964][web:962]. c3 **T3** (tracks mid-late stage). c4 **T1-T2, FAILS** — blind
...
**Tier assignment:** c1 **T3-T4**. c2 **T3** (non-specificity is well established, definitional). c3
**T2-T3** (ordering canonical but see AD-001 contest). c4 **T3** (N closest to cognition).
...
**Tier assignment:** c1 **T3-T4** (meta-analytic) [web:959]. c2 **T3** (AD-specific vs NfL). c3 **T3-T4**
(10-yr prognostic) [web:961]. c4 **T3** (real-world memory-clinic validation) [web:967].
...
**Tier assignment:** c1 **T3**. c2 **T2-T3** (partial independence from ATN plausible, not fully
established). c3 **T2** (AT(N)I is a proposed, not consensus, framing). c4 **T2-T3**.

**Verdict:** **SUGGESTIVE-to-SUPPORTED — a glial/inflammation axis carries partly-independent
...
GFAP). MechVal is looking less like "same template applied twice" and more like "one mechanistic
...
**Tier assignment:** c1 **T4** (unambiguous plaque removal). c2 **T3** (statistically robust, replicated
Phase 3). c3 **T2, CONTESTED** (magnitude below/near MCID; debated) [web:975]. c4 **T2** — proves amyloid
...
**Tier assignment:** c1 **T3-T4** (CSF engagement confirmed). c2 **T2-T3** (fluid tau moved). c3
...
**Tier assignment:** c1 **T3** (biomarker shifts real, statistically significant) [web:980]. c2
...
**The headline for Xia:** MechVal, pointed independently at both halves of Xia's program, re-derives the
